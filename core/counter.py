...
counter.py
──────────
极简持久化计数器（核心业务指标：总搜索、总访问、复制次数、成功交互、词频聚合）。
...
def _read_remote() -> tuple[int, int, int, int, dict]:
    ...
    try:
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            return r_total, r_visits, r_copies, r_successes, r_keywords
    except Exception as e:
        print(f"[Counter] 读取远端失败: {e}")
        return 0, BASE_VISITS, BASE_COPIES, 0, {}

def _sync_remote_task(adds_count, adds_visits, adds_copies, adds_successes, adds_keywords: dict) -> tuple[bool, int, int, int, int, dict]:
    repo_id, token = _get_config()
    if not repo_id or not token: return False, 0, 0, 0, 0, {}
    ...
    for _ in range(3):
        try:
            try:
                ...
                r_total, r_visits, r_copies, r_successes, r_keywords = 0, BASE_VISITS, BASE_COPIES, 0, {}
            ...
            api.upload_file(
                ...
                path_in_repo="count.json", repo_id=repo_id, repo_type="dataset", token=token,
                commit_message=f"Sync: 搜索:{n_total} |成功: {n_successes} | 复制:{n_copies} | 访问:{n_visits}️"
            )
            print(f"[Counter] ☁️ 同步成功！搜索:{n_total}, 成功交互:{n_successes}, 复制:{n_copies}")
            return True, n_total, n_visits, n_copies, n_successes, top_keywords

        except HfHubHTTPError as e:
            if "412 Precondition Failed" in str(e): time.sleep(1)
            else: break
...
async def _perform_sync():
    global _dirty_count, _dirty_visits, _dirty_copies, _dirty_successes, _dirty_keywords, _last_sync
    global _memory_count, _memory_visits, _memory_copies, _memory_successes, _memory_keywords

    lock = _get_sync_lock()
    if lock.locked(): return

    async with lock:
        if (_dirty_count + _dirty_visits + _dirty_copies + _dirty_successes + len(_dirty_keywords)) == 0: return

        c_adds, v_adds, cp_adds, s_adds = _dirty_count, _dirty_visits, _dirty_copies, _dirty_successes
        ...
        success, l_total, l_visits, l_copies, l_successes, l_keywords = await loop.run_in_executor(
            None, _sync_remote_task, c_adds, v_adds, cp_adds, s_adds, k_adds
        )

        if success:
            _dirty_count -= c_adds; _dirty_visits -= v_adds
            _dirty_copies -= cp_adds; _dirty_successes -= s_adds
...
async def init():
    global _memory_count, _memory_visits, _memory_copies, _memory_successes, _memory_keywords, _last_sync
    repo_id, token = _get_config()

    if not repo_id or not token: return
    loop = asyncio.get_running_loop()
    (
        _memory_count, _memory_visits, _memory_copies, _memory_successes, r_keys
...
def _check_sync():
    repo_id, token = _get_config()
    if repo_id and token and ((time.time() - _last_sync > SYNC_INTERVAL) or
       (_dirty_count + _dirty_visits + _dirty_copies + _dirty_successes + len(_dirty_keywords) >= SYNC_THRESHOLD)):
...
def get_hot_keywords(top_n=10) -> dict:
