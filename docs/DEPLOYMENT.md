...
>
> Docker 自托管部署（一键构建 / docker-compose）见 [DOCKER.md](./DOCKER.md)。
...
- 自定义绑定地址/端口（如 Docker、反向代理）：环境变量 `DANBOORU_HOST` / `DANBOORU_PORT`
...
DANBOORU_HOST=127.0.0.1               # 绑定地址（Docker / 反代场景改 0.0.0.0）
DANBOORU_PORT=11111                   # 绑定端口
```
...
| `DANBOORU_HOST` | 绑定监听地址（Docker / 反向代理场景设为 `0.0.0.0` 以对外暴露） | `127.0.0.1` |
| `DANBOORU_PORT` | 绑定监听端口 | `11111` |
...
2. **Docker 构建期烤进镜像**：在 `docker/Dockerfile` 里加一步触发一次 build（CPU 慢编一次，缓存留在镜像层），保证镜像自带缓存。
3. **运行时下载缓存**：代码内 `_pull_cloud_files()` 支持从 HF / OSS 拉取预构建的 `tags_embedding/*`，适合云部署。

> 注意：当前 `docker/Dockerfile` 使用 `pip install -r requirements.txt`。若要切到 uv 构建（推荐，与本地一致），可将安装步骤改为 `uv sync --no-dev` 并由 uv 生成的 `.venv` 提供运行环境。
